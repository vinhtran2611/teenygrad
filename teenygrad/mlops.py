...
class Sigmoid(Function):
    def forward(self, x: LazyBuffer) -> LazyBuffer:
        self.ret = x.const_like(1) / (
            x.const_like(1)
            + (x * x.const_like(-1 / math.log(2))).unary_op(UnaryOps.EXP2)
        )
        return self.ret
...
class Div(Function):
    ...
    def backward(
        ...
        return (
            ...
            (-grad_output * self.x) / (self.y * self.y)
...
class Where(Function):
    ...
    def backward(self, grad_output: LazyBuffer):
        return (
            None,
            self.x.ternary_op(TernaryOps.WHERE, grad_output, grad_output.const_like(0))
            if self.needs_input_grad[1]
            else None,
            self.x.ternary_op(TernaryOps.WHERE, grad_output.const_like(0), grad_output)
