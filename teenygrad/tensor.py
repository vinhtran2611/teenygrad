...
class Tensor:
    ...
    def assign(self, x) -> Tensor:
        ...
        if self.lazydata.realized is not None and not getenv("DISALLOW_ASSIGN"):
    ...
    def arange(start, stop=None, step=1, **kwargs):
        if stop is None:
            stop, start = start, 0
        return Tensor.full((ceil((stop - start) / step),), step, **kwargs).cumsum() + (
            start - step
        )
    ...
    def zeros_like(tensor, **kwargs):
        return Tensor.full_like(tensor, 0, **kwargs)

    @staticmethod
    def ones_like(tensor, **kwargs):
        return Tensor.full_like(tensor, 1, **kwargs)
    ...
    def pad(self, arg: Tuple[Tuple[int, int], ...], value: float = 0) -> Tensor:
        ...
        if isinf(value):
            return ret + copysign(1, value) / mlops.Pad.apply(
                Tensor.full(self.shape, value), arg=arg
            )
        return (
            ret
            if 0 == value
            else ret
            + (value - mlops.Pad.apply(Tensor.full(self.shape, value), arg=arg))
        )
    ...
    def __getitem__(self, val):
        ...
        ellipsis_idx = (
            len(orig_slices) if len(ellipses_found) == 0 else ellipses_found[0]
        )
        ...
        tensor_found = [
            (i, v) for i, v in enumerate(orig_slices) if isinstance(v, Tensor)
        ]
        ...
        valid_slices = [
            ...
            else slice(y := normalize_int(v, i, dim_sz), y + 1)
        ...
        if tensor_found:  # Fancy/tensor indexing
            for i, s in enumerate(sub):
                tensor_found[i] = (tensor_found[i][0] + s, tensor_found[i][1])
            dim = [i[0] for i in tensor_found]
            idx = [
                i[1].sign().contiguous().__neg__().contiguous().relu() * ret.shape[i[0]]
                + i[1]
                for i in tensor_found
            ...
            new_idx = idx[0].reshape(
                *[1] * dim[0], 1, *idx[0].shape, *[1] * (ret.ndim - dim[0] - 1)
            )
            arange = Tensor.arange(
                ret.shape[dim[0]], dtype=dtypes.int32, requires_grad=False
            ).reshape(
                *[1] * dim[0],
                ret.shape[dim[0]],
                *[1] * idx[0].ndim,
            ...
            for idx_, d in zip(idx[1:], sum_dim[1:]):
                new_idx = idx_.reshape(
                    *[1] * dim[0], *idx_.shape, *[1] * (ret.ndim - dim[0] - idx_.ndim)
                )
                arange = Tensor.arange(
                    ret.shape[d], dtype=dtypes.int32, requires_grad=False
                ).reshape(*[1] * (d), ret.shape[d], *[1] * (ret.ndim - d - 1))
            ...
            if (
                ...
                order = (
                    order[dim[0] : dim[0] + idx[0].ndim]
                    + order[: dim[0]]
                    + order[dim[0] + idx[0].ndim :]
                )
    ...
    def slice(
        self, arg: Sequence[Optional[Tuple[int, int]]], value: float = 0
    ) -> Tensor:
    ...
    def gather(self: Tensor, idx: Tensor, dim: int):
        ...
        return (
            (
                (idx == Tensor.arange(self.shape[dim]))
    ...
    def chunk(self, num, dim):
        slice_params = [
            [slice(None) for s in self.shape]
            for _ in range(ceil(self.shape[dim] / ceil(self.shape[dim] / num)))
        ]
    ...
    def argmax(self, axis=None, keepdim=False):
        if axis is None:
            return (
                prod(self.shape)
                - (
                    (self == self.max(axis)).flatten()
                    * Tensor.arange(prod(self.shape) - 1, -1, -1)
                ).max()
                - 1
            )
        ...
        m = self == (
            ...
            if keepdim
            else self.max(axis=axis, keepdim=keepdim).unsqueeze(axis)
        )
        idx = m * Tensor.arange(self.shape[axis] - 1, -1, -1).reshape(
            *[1] * axis, self.shape[axis], *[1] * (self.ndim - (axis + 1))
        )
    ...
    def _pool(
        ...
        if any(k > s for k, s in zip(k_, s_)) or any(d != 1 for d in d_):
            ...
            e_ = [
                ceil(k * (i + d) / i) for k, i, d in zip(k_, i_, d_)
    ...
    def log2(self):
        return mlops.Log.apply(self) / log(2)
    ...
    def triu(self, k: int = 0) -> Tensor:
        return Tensor._tri(self.shape[-2], self.shape[-1], k=k, dtype=self.dtype).where(
            self, Tensor.zeros_like(self)
        )

    def tril(self, k: int = 0) -> Tensor:
        return Tensor._tri(
            self.shape[-2], self.shape[-1], k=k + 1, dtype=self.dtype
    ...
    def _broadcasted(
        ...
        y: Tensor = (
            Tensor(
                cast(float, other), device=self.device, requires_grad=False, dtype=dtype
            )
            if other.__class__ is not Tensor
            else cast(Tensor, other)
        )
        if reverse:
            x, y = y, x
        if x.shape == y.shape:
    ...
    def sub(self, x: Union[Tensor, float], reverse=False) -> Tensor:
        return (
            ...
            if x.__class__ is Tensor or x or reverse
            else self
        )
    ...
    def div(self, x: Union[Tensor, float], reverse=False) -> Tensor:
        return (
            ...
            if x.__class__ is Tensor or reverse or not x
            else self.mul(1 / x)
        )
    ...
    def where(self: Tensor, input_: Union[Tensor, float], other: Union[Tensor, float]):
        ...
        y: Tensor = (
            ...
            if input_.__class__ is not Tensor
            else cast(Tensor, input_)
        )
        z: Tensor = (
            Tensor(
                cast(float, other), device=self.device, requires_grad=False, dtype=dtype
        ...
        if len_y_shape != max_shape:
            y = y.reshape((1,) * (max_shape - len_y_shape) + y.shape)
        if len_z_shape != max_shape:
            z = z.reshape((1,) * (max_shape - len_z_shape) + z.shape)
        ...
        if y.shape != shape_ret:
            y = y.expand(shape_ret)
        if z.shape != shape_ret:
            z = z.expand(shape_ret)
    ...
    def __lt__(self, x) -> Tensor:
        return self._broadcasted(mlops.Less, x, False)

    def __gt__(self, x) -> Tensor:
        return self._broadcasted(mlops.Less, x, True)
    ...
    def scaled_dot_product_attention(
        ...
        if is_causal:
            attn_mask = (
                Tensor.ones(self.shape[-2], key.shape[-2], requires_grad=False)
                .tril(0)
                .cast(dtypes.bool)
            )
        ...
        return (
            self @ key.transpose(-2, -1) / sqrt(self.shape[-1]) + attn_mask
        ).softmax(-1).dropout(dropout_p) @ value

    def sparse_categorical_crossentropy(self, Y, ignore_index=-1) -> Tensor:
        loss_mask = Y != ignore_index
        y_counter = (
            Tensor.arange(self.shape[-1], requires_grad=False)
            .unsqueeze(0)
